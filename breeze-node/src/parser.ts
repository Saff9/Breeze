...
export class Parser {
  ...
  private parseTry(): Statement {
    const tok = this.expect("KEYWORD", "try");
    this.expect("COLON");
    const body = this.parseBlockOrInline();
    let catchVar = "error";
    let catchBody: Statement[] = [];
    if (this.is("KEYWORD", "catch")) {
      this.advance();
      if (this.is("IDENT")) {
        catchVar = (this.advance().value as string) || "error";
      }
      this.expect("COLON");
      catchBody = this.parseBlockOrInline();
    }
    return { type: "Try", body, catchVar, catchBody, line: tok.line };
  }
